...
int main(int argc, char *argv[]) {
	...
	b->readpos = 0;
	b->writepos = 0;
	b->sem_read 	= sem_open("/AAU_T3_READ_SS2019", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 0);
	b->sem_write	= sem_open("/AAU_T3_WRITE_SS2019", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, BUFFERSIZE-1);
	mutex 		= sem_open("/AAU_T3_MUTEX_SS2019", O_CREAT | O_EXCL, S_IRUSR | S_IWUSR, 1);
	if (b->sem_read == SEM_FAILED || b->sem_write == SEM_FAILED || mutex == SEM_FAILED) {
		perror("sem_open");
		return EXIT_FAILURE;
	}
